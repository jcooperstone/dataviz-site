...
```{r xkcd null hypothesis, fig.alt = "A comit where two stick figures are talking. One says "I can't believe schools are still teaching kids about the null hypothesis. I remember reading a big study that conclusively disproved it years ago.", fig.cap= "Figure from [XKCD](https://www.explainxkcd.com/wiki/index.php/1478:_P-Values)", out.width = "70%", fig.align = "center", echo = FALSE}
...
```{r palmer penguins hex, fig.alt = "A hex sticker of 3 cute penguins with "palmer penguins" written in the background", fig.cap = "Illustration by [Allison Horst](https://allisonhorst.github.io/palmerpenguins/articles/art.html) out.width = "20%", fig.align = "center", echo = FALSE}
...
Is there a significant difference in the `body_weight_g` of male and female penguins?
...
```{r palmer penguins hex, fig.alt = "An orange normal distribution and a blue bimodal distribution, where the orange distribution tells the blue one it is not normal", fig.cap = "Illustration by [Allison Horst](https://github.com/allisonhorst/stats-illustrations) out.width = "70%", fig.align = "center", echo = FALSE}
knitr::include_graphics("https://github.com/allisonhorst/stats-illustrations/blob/main/other-stats-artwork/not_normal.png")
```
...
penguins_f_massbyspecies +
  annotate(geom = "text",
           x = c(1, 2, 3),
           y = 5 + body_mass_for_plotting$max_body_mass,
...
penguins_f_massbyspecies +
  geom_text(data = body_mass_for_plotting,
            aes(x = species,
                y = 100 + max_body_mass, 
                color = species, # group = species will also work
                label = groups))
```
...
In class, we will practice using ggplot and other extension packages to visualize correlations within a dataset.

You can find the in class content [here](3_07_correlations/07_correlations_recitation.html).
