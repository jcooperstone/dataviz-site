...
Today we will be mostly talking through [functions that life within the `dplyr` package](https://dplyr.tidyverse.org/reference/index.html).
...
```{r, eval = FALSE}
library(readxl)
sample_excel_data <- read_excel(file = "my-file-name.csv",
                                sheet = "Sheet1")
```
...
Sometimes you just want to see a vector in order. We can do that easily with [`arrange()`](https://dplyr.tidyverse.org/reference/arrange.html).
...
Although [`summarize()`](https://dplyr.tidyverse.org/reference/summarise.html) can be used without groups, I've included it here in this section since it is mostly used to calculate summary information group-wise. This function creates a wholly new dataframe, providing one row for each grouping variable. If there is no grouping, the resulting dataframe will have one row.

Let's look at an example. We can use `summarize()`
...
Generally it would be preferrable to use a `_join()` over `bind_cols()` or `bind_rows()` since the binding happens in the order that observations appear. This might make your data not meaningful without you knowing.
...
In class, we will practice wrangling our data, getting comfortable with rearranging it such that we can do this later before plotting. Click [here](04_wrangling_recitation.html) for the recitation materials.
