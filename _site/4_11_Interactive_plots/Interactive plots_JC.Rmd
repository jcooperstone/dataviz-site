...
## Why would you want to make an interactive plot?

Insert your apple boxplot here, and you can talk about how making interactive plots has been helpful in your research.
...
Data used here is `midwest` from `ggplot2` which contains demographic information from census data collected in the 2000 US census. The variable `percollege` includes what percentage of the respondants are college educated.
...
density <- ggplot(diamonds, aes(x = price)) + 
  ...
  labs(title = "Kernel density estimate with facets",
       x = "Price",
       y = "Density")
