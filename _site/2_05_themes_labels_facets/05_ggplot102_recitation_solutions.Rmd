...
What are the most popular breeds in 2020, two ways.
```{r most popular 2020}
breed_rank %>%
  filter(x2020_rank <= 5)
...
```{r all time most and least popular}
# most popular
breed_rank %>%
  rowwise() %>%
...
```{r all time most and least popular second way}
...
breed_rank <- bind_cols(breed_rank, breed_rank_2013to2020) %>%
  rename(rank_sum = "...12") %>%
  select(breed, rank_sum, everything())
...
breed_rank %>%
  ...
  select(breed, rank_sum, x2013_rank, x2020_rank)
