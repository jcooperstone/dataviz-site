...
library(readxl) # for rading in data
```

## Data
```{r}
emissions <- read_excel("data/co2_emissions_tonnes_per_person.xlsx")
```
...
## Visualization 1
https://www.gapminder.org/data/ CO2 emissions
```{r}
recent_emissions <- emissions %>%
  select(country, `2000`:`2017`)

recent_emissions_tidy <- recent_emissions %>%
  pivot_longer(cols = `2000`:`2017`,
               names_to = "year",
               values_to = "emissions")

recent_emissions_tidy %>%
  filter(year == 2000) %>%
...
Make a publication quality plot that shows the relationship between GDP per capita and life expectancy. Label the points with the name of the country for those that have the highest and lowest 
...
#  scale_y_continuous(labels = scales::dollar_format(), trans = "log2") +
  theme_minimal() +
