...
gwas <- read_csv("~/OneDrive - The Ohio State University/BuckeyeBox Data/JLC_Files/OSU/teaching/data-viz/extra_stuff/nph17693-sup-0007-tables16.csv") # be patient
```
...
Here, we are just modifying our plotting code slightly to allow it to be run across different features. Here, `feature_of_interest` is just the name I've assigned here, but you could pick whatever name you want.
...
Once we have our function written, we can use a function in the `apply()` family of functions (here, [`lapply()`](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/lapply) which applies and creates a list). Here I'm just printing the first 10 plots in the list
...
But you can print them all, save particular ones using`ggsave()`, write them all to a pdf, take each one and edit it more, or do anything else with them.
...
In class, we will practice making Manhattan plots. Click [here](10_manhattan_recitation.html) for the recitation materials.
