...
For those of you who are familiar with the tidyverse pipe `%>%`, you can also pipe the data to the ggplot function. When reading code, you can interpret the pipe as "and then." Here, take the `garden_harvest` data, and then, run `ggplot()`. Writing code in this way is my preference so I tend to code like this.
```{r data and pipe}
garden_harvest %>% 
  ggplot()
```
...
Shapes 0-20 accept only a `color` aesthetics. Shapes 21-25 accept both a `color` and `fill` aesthetic, where `color` controls the color of the outside of the shape, and `fill` controls the color of the inside of the shape. I basically always use shapes 21-25.
...
In class, we will practice using ggplot, especially playing around with writing syntax and invesetigating different geoms. Click [here](04_ggplot_recitation.html) for the recitation materials.
